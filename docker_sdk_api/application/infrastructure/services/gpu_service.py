        ...
        try:
            ...
            if len(gpus) == 0:
                ...
                for gpu in available_gpus:
                    gpus_dict[str(gpu.id)] = "GPU " + str(gpu.id) + " - " + str(gpu.name) + "- Available Memory: " + str(int(gpu.memoryFree)) + "MB/" + \
                                             str(int(gpu.memoryTotal)) + "MB"
                gpus_dict[str(-1)] = "CPU"
                return OrderedDict(gpus_dict)

        except Exception as e:
            raise GpuInfoInvalid(e.__str__())
