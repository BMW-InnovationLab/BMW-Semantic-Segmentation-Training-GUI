...
def resnet50_v1b(pretrained=False, root='~/.mxnet/models', ctx=cpu(0),pretrained_base_name='',classes=1000, **kwargs):
    ...
    use_global_stats : bool, default False
        ...
        optionally set to True if finetuning using ImageNet classification pretrained models.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], name_prefix='resnetv1b_',classes=classes, **kwargs)
    if pretrained_base_name!='':

        model.load_parameters(pretrained_base_name,ignore_extra=True,allow_missing=True,ctx=ctx)
        print("LOADING CLASSIFIER PARAMETERS")
    elif pretrained:
        print("Getting Pretrained Backbone ! ")
...
def resnet101_v1b(pretrained=False, root='~/.mxnet/models', ctx=cpu(0),pretrained_base_name='',classes=1000, **kwargs):
    ...
    use_global_stats : bool, default False
        ...
        optionally set to True if finetuning using ImageNet classification pretrained models.
    """
    model = ResNetV1b(BottleneckV1b, [3, 4, 23, 3], name_prefix='resnetv1b_', classes=classes,**kwargs)
    if pretrained_base_name!='':
        model.load_parameters(pretrained_base_name,ignore_extra=True,allow_missing=True,ctx=ctx)
        print("LOADING CLASSIFIER PARAMETERS")
    elif pretrained:
...
def resnet50_v1s(pretrained=False, root='~/.mxnet/models', ctx=cpu(0),pretrained_base_name='',classes=1000, **kwargs):
    ...
    model = ResNetV1b(BottleneckV1b, [3, 4, 6, 3], deep_stem=True, stem_width=64,
                      name_prefix='resnetv1s_', classes=classes,**kwargs)
    if pretrained_base_name!='':
        model.load_parameters(pretrained_base_name,ignore_extra=True,allow_missing=True,ctx=ctx)
        print("LOADING CLASSIFIER PARAMETERS")
    elif pretrained:
...
def resnet101_v1s(pretrained=False, root='~/.mxnet/models', ctx=cpu(0),pretrained_base_name='',classes=1000, **kwargs):
